...
impl<'info> WhitelistInvestor<'info> {
    fn whitelist(&self) -> Result<()> {
        require!(
            self.pool_master.key() == self.pool.pool_master,
            SynxError::UnauthorizedPoolMaster
        );
        Ok(())
    }
}

pub fn run_whitelist_investor(
...
pub struct InvestInPool<'info> {
    #[account(
        mut,
        has_one = treasury,
        constraint = pool.state_flags == WHITELISTED_ONLY || pool.state_flags == OPEN
    )]
    pub pool: Account<'info, Pool>,
    /// CHECK: This account only receives Lamports
    pub treasury: AccountInfo<'info>,
    /// CHECK: The investor account is used to sign transactions, ensuring that the investor has authorized the investment.
    #[account(mut)]
    pub investor: Signer<'info>,
    /// Optionally, include the whitelist entry account if in WHITELISTED_ONLY state
    #[account(
        mut,
        seeds = [b"WHITELIST", pool.key().as_ref(), investor.key().as_ref()],
...
pub fn run_invest_in_pool(ctx: Context<InvestInPool>, amount: u64) -> Result<()> {
    let pool = &mut ctx.accounts.pool;

    require!(
        pool.state_flags == WHITELISTED_ONLY || pool.state_flags == OPEN,
        SynxError::PoolNotAcceptingInvestments
    );

    // If in WHITELISTED_ONLY state, ensure the whitelist_entry is provided and the investor is whitelisted
    if pool.state_flags == WHITELISTED_ONLY {
        require!(
            ctx.accounts.whitelist_entry.investor == ctx.accounts.investor.key(),
            SynxError::InvestorNotWhitelisted
        );
    }

    // Logic to handle the investment (e.g., transferring funds to the treasury)
    // Implement your transfer logic here.

    msg!(
        "Investment of {} units received from {}",
        amount,
...
pub mod synx {
    ...
    pub fn invest_in_pool(ctx: Context<InvestInPool>, amount: u64) -> Result<()> {
        run_invest_in_pool(ctx, amount)?;
        sol_log_compute_units();
        Ok(())
    }
}
